...
The **control button** — the touch-native equivalent of a desktop Ctrl key, and the hub for control groups and selection editing — sits in the **bottom-left corner** (M4 playtest move; it began as the last button on the right edge). The corner keeps it under the left thumb while the other thumb works the command buttons on the right edge or the top-edge chips, so two-thumb combos like ctrl+move read naturally; while held it *changes what those gestures mean*:
